# Submission Plan

## Pre-Submission TODO

- [ ] **Join SoFiE** ($10 student membership) → Waives $65 JFEC submission fee
  - Link: https://sofie.stern.nyu.edu/membership
  - Benefits: JFEC online access, conference discounts, travel grants

---

## Primary Target: JFEC Special Issue

**Journal of Financial Econometrics - Special Issue on Machine Learning**
- **Deadline:** March 1, 2026
- **Impact Factor:** 2.2 (2024)
- **Submission Fee:** $65 (waived with SoFiE membership)
- **Fit:** Excellent - ML methods for financial econometrics

### Why JFEC?
1. Special issue specifically on ML in finance
2. Conformal prediction is novel in this space
3. Strong empirical contribution with 62 years of data
4. Clean, focused paper (18 pages)

---

## Alternative Venues (Ranked by Fit)

### Tier 1: Best Fit

| Journal | IF | Fit | Notes |
|---------|-----|-----|-------|
| **Quantitative Finance** | 1.5 | High | Factor investing, empirical focus |
| **J. Financial Data Science** | 1.0 | High | ML/AI in finance, PM Research |
| **J. Risk & Financial Mgmt** | 1.8 | High | Where Fantazzini (2024) published CP paper |

### Tier 2: Good Fit

| Journal | IF | Fit | Notes |
|---------|-----|-----|-------|
| **J. Computational Finance** | 0.9 | Medium | More numerical methods focus |
| **J. Finance & Data Science** | 3.9 | Medium | Elsevier/KeAi, broader scope |
| **J. Banking & Finance** | 3.5 | Medium | May need more economic motivation |

### Tier 3: Reach / Broader Audience

| Journal | IF | Fit | Notes |
|---------|-----|-----|-------|
| **Management Science** | 5.0 | Low | Very competitive, needs broader framing |
| **J. Financial Economics** | 8.2 | Low | Top-3, needs major theoretical contribution |
| **Review of Financial Studies** | 6.8 | Low | Top-3, very competitive |

---

## Venue Details

### 1. Quantitative Finance (Taylor & Francis)
- **Website:** https://www.tandfonline.com/journals/rquf20
- **Scope:** Mathematical and computational methods in finance
- **Review time:** ~3-6 months
- **Pro:** Strong fit for factor investing + uncertainty quantification
- **Con:** No special issue, regular submission

### 2. Journal of Financial Data Science (PM Research)
- **Website:** https://www.pm-research.com/content/iijjfds
- **Scope:** AI, ML, data science in investment management
- **Review time:** ~2-4 months
- **Pro:** Practitioner + academic audience, fast review
- **Con:** Lower IF (1.0)

### 3. Journal of Risk and Financial Management (MDPI)
- **Website:** https://www.mdpi.com/journal/jrfm
- **Scope:** Risk management, financial modeling
- **Review time:** ~1-2 months (fast)
- **Pro:** Already published conformal prediction paper (Fantazzini 2024)
- **Con:** Open access fees (~$2000), lower prestige

---

## Submission Strategy

### Option A: JFEC Only (Conservative)
1. Join SoFiE by January 2026
2. Submit to JFEC special issue by March 1, 2026
3. Wait for decision (~6-9 months)

### Option B: Parallel Submission Prep (Aggressive)
1. Submit to JFEC special issue (March 2026)
2. If rejected, immediately submit to Quantitative Finance
3. Prepare JFDS version as backup

### Option C: Dual Track (If Allowed)
Check if working paper on SSRN is allowed before JFEC submission
- Post to SSRN for visibility
- Submit to JFEC
- Build citations while under review

---

## Timeline

| Date | Action |
|------|--------|
| Jan 2025 | Join SoFiE ($10) |
| Feb 2025 | Post to SSRN (optional) |
| Feb 2026 | Final polish, prepare cover letter |
| Mar 1, 2026 | Submit to JFEC |
| Sep 2026 | Expected decision |
| Dec 2026 | Revision (if R&R) |

---

## References

- Fantazzini, D. (2024). Adaptive Conformal Inference for Computing Market Risk Measures. *J. Risk & Financial Management*, 17(6), 248.
- JFEC Special Issue CFP: https://academic.oup.com/jfec
- SoFiE Membership: https://sofie.stern.nyu.edu/membership
