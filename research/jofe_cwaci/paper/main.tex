...
\begin{enumerate}
    ...
    \item \textbf{Empirical documentation:} Using over 61 years of Fama-French factor data, we show that standard conformal prediction achieves only 74\% coverage during high-volatility periods versus the 90\% target. Volatility-scaled conformal prediction restores coverage to the target level, outperforming GARCH(1,1) prediction intervals (83--86\%) despite requiring no distributional assumptions.
...
Our work builds on the normalized conformal prediction literature \citep{papadopoulos2008normalized, lei2018distribution} by providing theoretical foundations specific to the multiplicative heteroskedasticity common in financial returns. While the technique of scaling nonconformity scores is not new, our contributions are: (1) rigorous analysis of \textit{when} and \textit{why} standard CP fails for factor returns, with explicit coverage bounds (Theorems~\ref{thm:undercover}--\ref{thm:robust}), (2) empirical demonstration that simple volatility scaling outperforms not only parametric GARCH methods but also conformalized quantile regression \citep{romano2019conformalized}---despite CQR being specifically designed for heteroskedastic data, and (3) comprehensive out-of-sample validation across 738 months showing robust performance where formal theoretical guarantees may not apply.
...
\end{align}
where $z_t \sim N(0,1)$ or $z_t \sim t_\nu$ (Student-t with $\nu$ degrees of freedom). We construct $(1-\alpha)$ prediction intervals as $\hat{\mu} \pm z_{1-\alpha/2} \cdot \hat{\sigma}_{t+1}$, where $\hat{\sigma}_{t+1}$ is the one-step-ahead volatility forecast. Models are re-estimated every 12 months using expanding windows.\footnote{Annual re-estimation balances parameter stability against adaptation to changing market conditions. Results are robust to quarterly or semi-annual re-estimation.}
...
\toprule
Factor & Standard CP & GARCH-N & GARCH-t & Vol-Scaled CP & VS vs GARCH-t \\
\midrule
Mkt-RF & 79.1\% & 83.4\% & 88.8\% & 89.3\% & $+$0.5pp \\
SMB & 81.2\% & 87.1\% & 90.9\% & 97.3\% & $+$6.4pp$^{*}$ \\
HML & 72.2\% & 82.4\% & 85.0\% & 88.8\% & $+$3.8pp \\
RMW & 65.8\% & 80.2\% & 84.5\% & 90.4\% & $+$5.9pp$^{*}$ \\
CMA & 74.9\% & 82.9\% & 83.4\% & 91.4\% & $+$8.0pp$^{**}$ \\
Mom & 72.7\% & 81.3\% & 84.5\% & 92.5\% & $+$8.0pp$^{**}$ \\
\midrule
\textbf{Average} & \textbf{74.3\%} & \textbf{82.9\%} & \textbf{86.2\%} & \textbf{91.6\%} & $+$5.4pp$^{**}$ \\
\textbf{Gap from 90\%} & $-$15.7pp & $-$7.1pp & $-$3.8pp & $+$1.6pp & --- \\
\bottomrule
\end{tabular}
\begin{flushleft}
\small\textit{Note:} $^{***}p<0.001$, $^{**}p<0.01$, $^{*}p<0.05$ for two-proportion z-test. VS vs GARCH-t column tests whether Vol-Scaled CP significantly outperforms GARCH with Student-t innovations.
\end{flushleft}
\end{table}
...
Conformalized quantile regression \citep{romano2019conformalized} was specifically designed to handle heteroskedastic data, yet achieves only 74.1\% high-volatility coverage---no better than standard CP. This counterintuitive result deserves explanation.

CQR works by training quantile regression models to predict conditional quantiles $\hat{q}_{\alpha/2}(x)$ and $\hat{q}_{1-\alpha/2}(x)$, then conformally calibrating these estimates. The key limitation is \textit{extrapolation}: CQR learns the relationship between features and quantiles from calibration data. When test-period volatility substantially exceeds the calibration-period range, CQR must extrapolate beyond its training distribution.

Consider the factor return setting:
\begin{enumerate}
    \item \textbf{Limited features.} With only lagged returns and volatility as predictors, quantile regression has limited capacity to capture the full conditional distribution.

    \item \textbf{Regime extrapolation.} The 2008 financial crisis and 2020 COVID crash produced volatility levels rarely seen in earlier calibration data. CQR's learned quantiles, fitted on 1963--1993 data, cannot anticipate these extremes.

    \item \textbf{Conformal correction is local.} CQR's conformal calibration step adjusts for \textit{average} miscalibration, not volatility-conditional miscalibration. If quantile regression systematically underestimates uncertainty during high volatility, the single scalar correction cannot fix this.
\end{enumerate}

In contrast, volatility-scaled CP applies a \textit{multiplicative} correction: intervals scale proportionally with current volatility, regardless of whether that volatility level appeared in calibration. This multiplicative structure matches the data-generating process (Assumption~\ref{ass:het}), enabling correct coverage even under regime changes.
...
\begin{enumerate}
    \item \textbf{Factors with theoretical guarantees:} Mkt-RF and Momentum satisfy the i.i.d.\ assumption. For these factors, our coverage guarantees hold, and empirical results (89--92\% high-vol coverage) match theory.

    \item \textbf{Factors without formal guarantees:} SMB, HML, RMW, and CMA violate i.i.d. For these factors, our theoretical results should be interpreted as \textit{heuristic guidance} rather than formal guarantees. The method still achieves 86--97\% high-volatility coverage empirically, but we cannot claim finite-sample validity.
\end{enumerate}
