...
\begin{algorithmic}[1]
\REQUIRE Source data $\mathcal{D}_S$ with crowding labels, Target data $\mathcal{D}_T$ unlabeled
\REQUIRE MMD weight $\lambda$ (set via cross-validation, typically 0.1-1.0)
\FOR{each epoch}
    \FOR{each batch $(X_S, Y_S), (X_T)$}
        \STATE $F_S \gets$ FeatureExtractor$(X_S)$ \COMMENT{Extract source features}
        \STATE $F_T \gets$ FeatureExtractor$(X_T)$ \COMMENT{Extract target features}
        \STATE $\mathcal{L}_{\text{task}} \gets$ CrossEntropy(Classifier$(F_S)$, $Y_S$) \COMMENT{Source task loss}
        \STATE $\mathcal{L}_{\text{MMD}} \gets \text{MMD}(F_S, F_T)$ \COMMENT{Global MMD (no regime conditioning)}
        \STATE $\mathcal{L}_{\text{total}} \gets \mathcal{L}_{\text{task}} + \lambda \cdot \mathcal{L}_{\text{MMD}}$
        \STATE Update feature extractor parameters to minimize $\mathcal{L}_{\text{total}}$
    \ENDFOR
\ENDFOR
\end{algorithmic}
\end{algorithm}
