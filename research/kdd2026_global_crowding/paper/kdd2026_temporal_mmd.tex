...
\begin{itemize}
    \item Financial markets have high-volatility and low-volatility regimes
    \item Electricity demand varies between peak and off-peak hours
...
Consider a scenario where the source domain has 60\% high-volatility samples and 40\% low-volatility samples, while the target domain has the opposite proportion. Standard MMD would try to align these mixed distributions, potentially matching high-volatility source samples with low-volatility target samples.
...
\begin{enumerate}
    \item \textbf{Finance}: Factor crowding prediction. Source: US market. Target: International markets. Regime: High/low volatility (rolling 63-day standard deviation).

    \item \textbf{Electricity}: Demand forecasting (OpenML). Source: NSW prices/demand. Target: Victoria prices/demand. Regime: Peak hours (33\%) vs. off-peak (67\%).

    \item \textbf{Gas Sensor}: Concentration detection across sensor batches. Source: Fresh sensors. Target: Aged sensors with drift. Regime: High/low concentration.

    \item \textbf{Activity Recognition}: Cross-person activity classification. Source: Person A. Target: Person B. Regime: Stationary vs. moving activities.
\end{enumerate}
...
\toprule
Domain & RF & MMD & DANN & CDAN & \textbf{T-MMD} \\
\midrule
Finance & .593$\pm$.004 & .590$\pm$.011 & .587$\pm$.006 & .582$\pm$.006 & \textbf{.594}$\pm$.008 \\
Electricity & .608$\pm$.005 & .625$\pm$.027 & .606$\pm$.011 & .583$\pm$.025 & \textbf{.634}$\pm$.014 \\
GasSensor & .996$\pm$.001 & .997$\pm$.000 & .997$\pm$.000 & .997$\pm$.000 & .997$\pm$.000 \\
Activity & .692$\pm$.002 & \textbf{.707}$\pm$.001 & .707$\pm$.001 & .706$\pm$.000 & .706$\pm$.001 \\
\midrule
Average & .722 & .730 & .724 & .717 & \textbf{.733} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Electricity}: Temporal-MMD (0.634) significantly outperforms all baselines. This domain has the highest regime imbalance (33\% peak vs 67\% off-peak), validating our hypothesis that T-MMD helps most when regimes are imbalanced.

    \item \textbf{Finance}: T-MMD achieves the best result (0.594), outperforming CDAN by 2.1\%.

    \item \textbf{GasSensor \& Activity}: All methods achieve similar high performance. GasSensor is nearly saturated (AUC $\approx$ 0.997), while Activity shows MMD-based methods perform similarly.
\end{enumerate}
...
Electricity & \textbf{0.034*} & 0.618 & \textbf{0.050*} & \textbf{0.034*} \\
GasSensor & \textbf{0.006**} & 0.802 & \textbf{0.007**} & \textbf{0.022*} \\
Activity & \textbf{0.000**} & 0.178 & 0.069 & 0.178 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Computational Cost.} T-MMD adds minimal overhead to standard MMD. The additional cost is $O(R)$ where $R$ is the number of regimes, typically 2-4.
