...
\subsection{8.1 Robustness of Game-Theoretic Model}
...
| Factor | Hyperbolic R² | Exponential R² | \textbf{Winner} | BIC Difference |
|--------|---------------|----------------|-----------|----|
...
\textbf{Alternative 2: Time vs. Crowding}
...
| HML | 0.71 | 0.58 | 0.75 |
| MOM | 0.74 | 0.61 | 0.79 |
\textbf{Finding}: Time-based model outperforms crowding-only model. Combined model (time + crowding) performs best. This suggests that both exogenous decay (over time) and endogenous crowding (capital flows) are important.
\textbf{Alternative 3: Decay Parameter Stability}
...
\textbf{Finding}: Decay rates show moderate variation (CV ~0.2) but consistent upward trend. This is consistent with the hypothesis that increasing competition in factor investing accelerates decay rates over time.
\subsection{8.2 Robustness of Temporal-MMD}
\textbf{Regime Definition Sensitivity}
We test whether Temporal-MMD is sensitive to how regimes are defined. We try three regime definitions:
\textbf{Regime Set 1} (Baseline): Bull/Bear + High/Low Vol (4 regimes)
\textbf{Regime Set 2}: Market Return Percentile (3 regimes: bottom 33%, middle 33%, top 33%)
\textbf{Regime Set 3}: Volatility Only (2 regimes: vol above/below median)
\textbf{Transfer Efficiency Results}:
...
\textbf{Finding}: Transfer efficiency is highest with the baseline regime definition but remains strong (>58%) across all specifications. The 2×2 grid (bull/bear × high/low vol) is optimal.
\textbf{Kernel Selection}
We test alternative kernels for MMD computation:
\textbf{Kernels Tested}:
1. RBF (Gaussian) - baseline
2. Polynomial (degree 2)
3. Laplacian
4. Multiple kernels (weighted combination)
\textbf{Results (Average TE across markets)}:
...
| Laplacian | 0.619 |
| Multi-kernel | 0.641 |
\textbf{Finding}: RBF kernel (baseline) and multi-kernel approach perform best. Results are stable to kernel choice (all >0.61), suggesting the regime conditioning matters more than the specific kernel.
\subsection{8.3 Robustness of CW-ACI}
...
\subsection{8.4 Cross-Validation and Overfitting Checks}
...
| Temporal-MMD | 0.58 | 0.62 | 0.65 | 0.61 | 0.57 | \textbf{0.61} |
...
\subsection{8.5 Generalization to Other Asset Classes}
\textbf{Test 1: Factor Investing in Fixed Income}
...
\textbf{Test 2: Commodity Factor Investing}
We test on commodity factors (carry, momentum, value in commodity markets).
Results: Decay rates are higher for commodities ($\lambda$_commodity ≈ 1.5× $\lambda$_equity), likely due to lower liquidity. Temporal-MMD works but with reduced efficiency (TE = 0.54 vs. 0.64 for equity).
\textbf{Test 3: Cryptocurrency Returns}
We test on Bitcoin and Ethereum (30 factors, 2015–2024).
Results: Crypto factors show much faster decay ($\lambda$ = 0.3–0.5 per month vs. 0.05–0.20 per year for equity). CW-ACI hedging works but requires more frequent rebalancing.
\textbf{Finding}: Core framework generalizes to other assets, with parameter values scaling appropriately for liquidity/volatility differences.
\subsection{8.6 Discussion: Limitations and Future Work}
\textbf{Limitations of Current Work}
1. \textbf{Crowding Measurement}: Proxies from returns may have feedback loops with factor performance. Ideal measurement uses direct AUM data, which is proprietary.
2. \textbf{Mechanistic Game Theory}: While we derive decay from equilibrium, real investor behavior is more complex (loss aversion, herding, institutional constraints).
3. \textbf{Regime Definition}: Fixed regimes may miss dynamic regime shifts. Hidden Markov models could improve regime classification.
4. \textbf{Transaction Costs}: Hedging analysis assumes static option prices. In practice, option prices widen during crashes.
5. \textbf{Convergence to Equilibrium}: We assume markets reach equilibrium quickly. In reality, adjustment lags could be significant.
\textbf{Future Research Directions}
1. \textbf{Agent-Based Models}: Simulate heterogeneous agents with learning and loss aversion to validate game-theoretic predictions
2. \textbf{Network Analysis}: Model factor crowding as a network problem (shared holdings, systemic risk)
3. \textbf{Real-Time Crowding Measurement}: Use regulatory filings (13F) and prime brokerage data for direct AUM measurement
4. \textbf{Multi-Factor Hedging}: Optimize hedge portfolio across multiple factors simultaneously
5. \textbf{Causal Inference}: Use instrumental variables (e.g., policy shocks) to establish causal effects of crowding on decay
\subsection{8.7 Broader Implications}
...
\textbf{Key Tests}: Model specification, regime definition, weight functions, cross-validation, generalization
\textbf{Robustness Summary}:
\begin{itemize}
  \item Hyperbolic decay beats exponential across all factors
  \item Temporal-MMD transfer efficiency robust to regime/kernel choices (0.59–0.64)
  \item CW-ACI maintains coverage guarantee across prediction horizons
  \item Time-series CV shows no overfitting (OOS R² ~50–60%)
  \item Framework generalizes to fixed income, commodities, crypto
\end{itemize}

\textbf{Figures Referenced}:
\begin{itemize}
  \item Figure 19: Model specification comparison
  \item Figure 20: Cross-validation performance
  \item Figure 21: Generalization to other assets
\end{itemize}

\textbf{Tables Referenced}: Tables 11–16 (robustness analyses)