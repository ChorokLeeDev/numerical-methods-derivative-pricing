...
This equilibrium satisfies $C^	extit{(0) = 0$ and $\frac{dC^}(t)}{dt} \geq 0$ for all $t$.
...
$$K(t) - \lambda_0 C^	extit{(t) = \text{TC}(C^}(t)) + r_f$$

Rearranging:
$$K(t) - r_f = \lambda_0 C^	extit{(t) + \text{TC}(C^}(t))$$
...
By the Intermediate Value Theorem (since $F$ is continuous in $C$ by Assumption A2), there exists at least one $C^	extit{ \in [0, C_{\max}]$ such that $F(C^}, t) = 0$.
...
$$C^	extit{(t) = \frac{1}{\lambda_0 + \text{TC}'(C^}(t))} [K(t) - r_f]$$
...
However, as $K(t)$ decays, $C^	extit{(t)$ adjusts downward. At each instant, crowding is at its equilibrium level given current $K(t)$. Since $K(t)$ is decreasing and the equilibrium $C^}$ is increasing in $K$, the path $C^*(t)$ is 	extbf{non-increasing} (weakly decreasing).

Wait, I need to be more careful. Let me reconsider the dynamics.

	extbf{Revised Step 4: Dynamic adjustment}

If crowding is below equilibrium ($C < C^	extit{$), then $\alpha - \text{TC} > r_f$, so new capital flows in ($\frac{dC}{dt} > 0$). If crowding is above equilibrium ($C > C^}$), then $\alpha - \text{TC} < r_f$, so capital exits ($\frac{dC}{dt} < 0$).

This is consistent with standard adjustment dynamics. At equilibrium, $C(t) = C^	extit{(t)$, and as $K(t)$ decays, the equilibrium $C^}(t)$ also decays. With instantaneous adjustment, $C(t)$ tracks $C^*(t)$ exactly, so $C(t)$ is decreasing with decreasing $K(t)$.

More formally, from Implicit Function Theorem applied to $F(C, t) = 0$:
$$\frac{dC^	extit{}{dt} = -\frac{\partial F/\partial t}{\partial F/\partial C} = -\frac{\frac{\partial}{\partial t}[K(t) - r_f]}{\lambda_0 + \text{TC}'(C^})} = -\frac{K'(t)}{\lambda_0 + \text{TC}'(C^*)} < 0$$
...
$$\alpha_{\text{obs}}(t) = K(t) - \lambda_0 C^	extit{(t) = K(t) - \lambda_0 C^}(t)$$

where $C^	extit{(t)$ solves $K(t) - \lambda_0 C^} = \text{TC}(C^*) + r_f$.

For the linear TC case $\text{TC}(C) = \alpha_0 + \beta C$ (linear in crowding), we have:
$$K(t) - \lambda_0 C^	extit{ = \alpha_0 + \beta C^} + r_f$$
...
$$\frac{d \alpha_{\text{obs}}}{dt} = \frac{\partial \alpha}{\partial K} \frac{dK}{dt} + \frac{\partial \alpha}{\partial C^	extit{} \frac{dC^}}{dt}$$

We have:
- $\frac{\partial \alpha}{\partial K} = 1$ (since $\alpha = K - \lambda_0 C^	extit{$, and $C^}$ depends on $K$)
...
$$C^	extit{ = \frac{K(t) - r_f - \text{TC}^{-1}(\text{TC}(C^}))}{\lambda_0}$$
