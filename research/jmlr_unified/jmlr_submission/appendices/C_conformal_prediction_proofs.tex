...
\textbf{Lemma C.1} 	\textit{(Exchangeability Preservation)}: If sequence $\{(x_i, y_i, C_i)\}_{i=1}^{n+1}$ is exchangeable and $C \perp y | x$, then the weighted sequence with weights $w_i = \sigma(C_i)$ remains exchangeable.
