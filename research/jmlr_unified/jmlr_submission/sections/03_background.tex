...
3. \textbf{ST_Rev (Short-Term Reversal)}: Monthly reversal
...
4. \textbf{LT_Rev (Long-Term Reversal)}: 2-5 year reversal
...
Formally, let $i$ be a player with strategy $s_i \in S_i$ and payoff $u_i(s_i, s_{-i})$. A strategy profile $(s_1^\textit{, \ldots, s_n^})$ is a Nash equilibrium if:
$$u_i(s_i^\textit{, s_{-i}^}) \geq u_i(s_i, s_{-i}^*) \quad \forall i, \forall s_i \in S_i$$
