...
\subsection{4.2 Derivation of Hyperbolic Decay}
...
\textbf{Key Assumption: Alpha Decay in Crowding}
We assume that the \textit{intrinsic} alpha of the factor (when uncrowded) decays exogenously over time. This could be due to:
\begin{itemize}
  \item Market adaptation (more people learning about the factor)
  \item Factor publication effect (documented by McLean \& Pontiff, 2016)
  \item Technological diffusion (tools that exploit the factor become widely available)
\end{itemize}
...
$$\alpha_{\text{obs}}(t) = K(t) - \lambda_0 \cdot C^\textit{(t) = \frac{K_0}{1 + \gamma t} - \lambda_0 \cdot C^}(t)$$
...
This result—that observed alpha converges to the risk-free rate—is correct at steady state but hides the dynamics. The empirically observable quantity is the \textit{realized} alpha before full adjustment:
...
The hyperbolic form emerges because of the \textit{linear} relationship between crowding and transaction costs combined with \textit{capital inflow proportional to profitability}. The interaction creates a self-stabilizing dynamic: as alpha declines, inflows slow, which slows further crowding, which slows alpha decay. This creates hyperbolic rather than exponential decay.
\subsection{4.3 Formal Theorems and Proofs}
\textbf{Theorem 1: Existence and Uniqueness of Equilibrium}
\textit{Statement}: Consider the crowding game with investor payoff function $\Pi_j(w_j, C, t)$ and entry condition $\alpha(t) \geq \lambda_0 \cdot C(t) + r_f$. Under the assumption that $\alpha(t)$ decays exogenously as $\alpha(t) = K(t) - \lambda_0 \cdot C(t)$ with $K(t)$ continuously differentiable and $K(t) \geq r_f$ for all $t \geq 0$, there exists a unique equilibrium crowding path $C^*(t)$ satisfying the indifference condition at all times $t$.
\textit{Proof Sketch}: (Full proof in Appendix A)
\begin{itemize}
  ...
  \item Solving for $C^\textit{$: $C^}(t) = \frac{1}{\lambda_0} \left( \frac{K(t)}{1 + \gamma t} - r_f \right)$
  \item Uniqueness follows from the monotonic relationship between $C$ and $\alpha$.
\end{itemize}

\textbf{Theorem 2: Properties of Decay Rate}
\textit{Statement}: In the equilibrium of Theorem 1, the observed alpha decay rate parameter $\lambda$ satisfies:
...
\textbf{Theorem 3: Heterogeneous Decay Between Mechanical and Judgment Factors}
\textit{Statement}: Consider two factors: a mechanical factor $M$ and a judgment factor $J$. Suppose the barrier to entry is lower for mechanical factors (smaller $\lambda_{0,M}$) but the exogenous decay rate is faster for judgment factors (larger $\gamma_J$). Then:
$$\lambda_J > \lambda_M$$
...
\subsection{4.5 Bridge to Empirical Validation}
...
\textbf{Key Theorems}: Theorem 1 (Existence/Uniqueness), Theorem 2 (Decay Properties), Theorem 3 (Heterogeneous Decay)
\textbf{Proofs Location}: Appendix A (detailed proofs of all three theorems)
\textbf{Figures Referenced}: Figure 7 (equilibrium dynamics), Figure 8 (decay curves by factor type)
\textbf{Tables Referenced}: Table 4 (parameter estimates), Table 5 (heterogeneity test results)