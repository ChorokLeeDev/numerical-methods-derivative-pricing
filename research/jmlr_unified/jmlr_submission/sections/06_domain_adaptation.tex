...
Why might US factors not transfer directly to foreign markets? The key issue is \textit{regime shifts}.
...
Standard domain adaptation tries to match these distributions uniformly, which forces incompatible regimes to align. This can \textit{hurt} transfer performance.
\textbf{The Solution: Regime Conditioning}
The key innovation is to identify market regimes and match distributions \textit{within each regime separately}. This ensures:
\begin{itemize}
...
The key innovation of Temporal-MMD is to partition data by regime and compute MMD \textit{within each regime}.
...
\textit{Statement}: Suppose source and target distributions can be partitioned into regimes $R$ such that within-regime distributions are close (small MMD). Then the target error of a model trained on source with Temporal-MMD adaptation satisfies:
...
\textit{Implication}: The bound is tighter with regime conditioning because we replace the global MMD (large due to regime shifts) with regime-specific MMD values (smaller because within-regime distributions are closer).
\textit{Proof Sketch}: (Full proof in Appendix B)
\begin{itemize}
...
\textbf{Synergy}: Game theory explains \textit{why} regimes matter (investor behavior changes), and Temporal-MMD operationalizes this insight in domain adaptation.
---
