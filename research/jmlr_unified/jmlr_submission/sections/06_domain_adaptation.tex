...
\subsection{Standard MMD framework}
...
\subsection{Empirical validation: global transfer}
...
\hline
\textbf{Transfer} & \textbf{Baseline} & \textbf{Direct} & \textbf{MMD} & \textbf{Improve} \\
\hline
...
\hline
\textbf{Avg} & \textbf{0.557} & \textbf{0.386} & \textbf{0.600} & \textbf{+7.7\%} \\
\hline
\end{tabular}
}
\end{center}
...
\subsection{MMD-based transfer bound}

\begin{theorem}[Domain Adaptation Error Bound]
\label{thm:domain-adaptation-bound}
(Ben-David et al., 2010; adapted from Long et al., 2015) Suppose the source domain and target domain have distributions $P_S$ and $P_T$. Let $h$ be a hypothesis (model) trained on source data, and let $\lambda(\text{MMD}(P_S, P_T))$ denote the domain discrepancy. Then the target error satisfies:
...
\textbf{Implication}: By minimizing MMD during domain adaptation, we reduce the gap between source and target distributions, thereby tightening the bound on target error. This justifies the use of MMD-based transfer learning: smaller domain discrepancy leads to better target performance guarantees.

\textbf{Proof Sketch}: (Full proof in Appendix B, adapted from Ben-David et al., 2010)
\begin{itemize}
    ...
    \item $\mathcal{H}$-divergence $d_\mathcal{H}(P_S, P_T)$ upper-bounds the difference in error distributions
    \item MMD provides an estimate of $\mathcal{H}$-divergence in kernel space
    \item Minimizing MMD directly reduces this upper bound on target error
\end{itemize}

\subsection{Connection to game-theoretic model}
