...
7. \textbf{Output}: Prediction set $\mathcal{C}(x_{n+1})$ with guaranteed coverage
...
\textit{Statement}: Under the assumption that crowding is independent of the outcome conditional on the features (i.e., $C \perp y | x$), the CW-ACI prediction set $\mathcal{C}$ satisfies:
...
\textit{Proof Sketch}: (Full proof in Appendix C)
...
\begin{itemize}
  ...
  \item Weighted extension: If crowding is independent of outcome conditional on features, then the weighted sample remains exchangeable
  \item Weighted quantile of exchangeable data maintains $(1 - \alpha)$ quantile property
  \item Therefore, coverage is preserved
\end{itemize}

\textit{Assumption Check}: We verify the conditional independence assumption using:
\begin{itemize}
  \item Permutation tests on $(C_i, A_i)$ residuals
  \item Mutual information analysis: $I(C; y | x) \approx 0$
\end{itemize}
