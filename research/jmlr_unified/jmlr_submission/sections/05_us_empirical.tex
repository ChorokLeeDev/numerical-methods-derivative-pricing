...
\subsection{5.1 Data and Methodology}
\textbf{Factor Data}
...
\subsection{5.2 Results: Parameter Estimation}
...
| \textbf{ST_Rev} | Judgment | 6.14 | [5.28, 7.00] | 0.218 | [0.174, 0.262] | 0.77 | 0.63 |
| LT_Rev | Judgment | 3.46 | [2.81, 4.11] | 0.127 | [0.091, 0.163] | 0.65 | 0.52 |
\textbf{Key Findings}:
1. \textbf{Heterogeneous Decay Rates}: Judgment factors ($\lambda_J = 0.173 \pm 0.025$, mean ± std) decay significantly faster than mechanical factors ($\lambda_M = 0.072 \pm 0.010$). The difference is 2.4× ($p < 0.001$).
2. \textbf{Profitability Scale}: Judgment factors have higher initial alpha ($K_J = 4.84\%$ vs. $K_M = 3.30\%$), but this is offset by faster decay.
3. \textbf{Model Fit}: The hyperbolic model explains 59–77% of in-sample variance and 45–63% of out-of-sample variance. This is strong for financial data.
4. \textbf{Momentum Factor}: The momentum factor exhibits the fastest decay ($\lambda_{\text{MOM}} = 0.192$), consistent with the "momentum crash" literature (Bender et al., 2013).
\textbf{Theorem 7 Test: Heterogeneous Decay}
\textit{Hypothesis}: $\lambda_{\text{judgment}} > \lambda_{\text{mechanical}}$
\textit{Test Method}: Mixed-effects regression with factor type as predictor:
...
\subsection{5.3 Out-of-Sample Validation}
...
\subsection{5.4 Heterogeneity Analysis}
...
\textbf{Key Pattern}: Decay rates tend to increase over time for most factors, suggesting that as the factor universe becomes more crowded overall, individual factors decay faster. This is consistent with theory: as the investment industry grows, competition for factors intensifies.
\textbf{Factor Characteristics and Decay Rate}
...
\textbf{Results}:
\begin{itemize}
  \item \textbf{Turnover effect}: Factors requiring higher turnover decay faster ($\beta_1 = 0.024, p = 0.08$)
  \item \textbf{Correlation effect}: Factors highly correlated with market beta decay slower ($\beta_2 = -0.015, p = 0.12$)
  \item \textbf{Judgment effect}: Judgment factors decay faster ($\beta_3 = 0.101, p < 0.001$)
\end{itemize}
...
\begin{itemize}
  \item Hyperbolic decay model explains 45–63% of OOS variance
  \item Judgment factors decay 2.4× faster than mechanical factors ($p < 0.001$)
  \item Decay rates have increased over time as investment industry grows
  \item Out-of-sample predictive power ~55% average
\end{itemize}

\textbf{Figures Referenced}:
\begin{itemize}
  \item Figure 9: Decay curves by factor type
  \item Figure 10: OOS R² comparison across periods
  \item Figure 11: Time evolution of decay rates
\end{itemize}

\textbf{Tables Referenced}: Table 4 (parameter estimates), Table 5 (OOS R²), Table 6 (sub-period analysis)