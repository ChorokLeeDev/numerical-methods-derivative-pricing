...
Figure~\ref{fig:model_fit} illustrates this contrast visually. For momentum (left panel), the hyperbolic curve tracks the rolling Sharpe ratio's decline from $\sim$1.5 in the mid-1990s to $\sim$0.25 today. For value (right panel), no model captures the erratic pattern---the factor oscillates without systematic decay.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\columnwidth]{figures/icaif_fig1_model_fit.png}
\caption{Model fit comparison: Momentum (mechanical) exhibits clear hyperbolic decay ($R^2 = 0.65$); Value (judgment) shows no systematic decay pattern ($R^2 = 0.05$).}
\label{fig:model_fit}
\end{figure}
...
We test three strategies (Table~\ref{tab:strategies}):

\begin{table}[h]
\centering
\small
\caption{Strategy performance (2017--2025)}
\label{tab:strategies}
\begin{tabular}{lc}
\toprule
Strategy & Sharpe \\
\midrule
Factor Momentum & \textbf{0.39} \\
Crowding-Timed & 0.22 \\
Equal Weight & 0.17 \\
\bottomrule
\end{tabular}
