\subsection{Financial notation and factor definitions}
...
\subsection{Domain adaptation and maximum mean discrepancy}
...
\textbf{MMD for Domain Adaptation}
Standard MMD matches $P_S$ and $P_T$ by minimizing distributional distance in a learned representation space. For domain adaptation, we minimize:
$$\mathcal{L} = \mathcal{L}_{\text{task}}(S, y_S; f_\theta) + \lambda \cdot \text{MMD}^2(f_\theta(S), f_\theta(T))$$
where $\mathcal{L}_{\text{task}}$ is the supervised loss on source data, $f_\theta$ is a learned feature extractor, and $\lambda$ balances task performance with domain alignment.
This ensures that source and target distributions are aligned in the learned representation space, enabling transfer of models trained on source data to perform well on target data.
