...
\textbf{Theorem 5} 	\textit{(Domain Adaptation Transfer Bound)}: Let $S$ be a source domain and $T$ be a target domain, both partitionable into regimes $R = \{r_1, \ldots, r_K\}$. Let $h: X \to Y$ be a hypothesis (predictor). Define:
\begin{itemize}
\item $\text{Error}_S(h)$ = expected loss on source data
\item $\text{Error}_T(h)$ = expected loss on target data
\item $\text{MMD}_r(S, T)$ = Maximum Mean Discrepancy between source and target in regime $r$
\end{itemize}
...
Let $X$ be the input space and $Y$ the output space. A hypothesis $h: X \to Y$ has loss $\ell(h(x), y) \in [0, 1]$. Define:
\begin{itemize}
\item \textbf{Source loss}: $\text{Error}_S(h) = \mathbb{E}_{(x,y) \sim P_S}[\ell(h(x), y)]$
\item \textbf{Target loss}: $\text{Error}_T(h) = \mathbb{E}_{(x,y) \sim P_T}[\ell(h(x), y)]$
\end{itemize}
...
where:
\begin{itemize}
\item $H\Delta H_{S,r,T,r}(h)$ = the $H$-divergence between source and target in regime $r$ (measures distribution mismatch)
\item $\text{Disc}_{S,r,T,r}(h)$ = the regime-specific discrepancy (due to factors specific to that regime)
\end{itemize}
...
\textbf{Input}:
\begin{itemize}
\item Time series of factor returns $\{\alpha_t\}_{t=1}^T$
\item Historical excess market returns $\{m_t\}_{t=1}^T$
\end{itemize}

\textbf{Parameters}:
\begin{itemize}
\item Window size: $w$ (e.g., 60 months for 5-year rolling)
\item Percentile threshold: $p$ (e.g., 0.5 for median)
\end{itemize}
...
\subsubsection{Algorithm B.2: Standard MMD Optimization (Used in Section 6)}

\textbf{Input}:
\begin{itemize}
\item Source labeled data: $S = \{(x_i, y_i)\}_{i=1}^{n_S}$
\item Target unlabeled data: $T = \{x_j'\}_{j=1}^{n_T}$
\item Feature extractor network $f_\theta(x)$ with parameters $\theta$
...
\textbf{Output}: Trained feature extractor $f_\theta^*$ and predictor $p_{w^*}$


\subsection{Summary}

Appendix B provides theoretical foundations for MMD-based domain adaptation. Theorem 5 proves that regime-conditional domain adaptation can provide tighter error bounds than global MMD, motivating sophisticated approaches. However, empirically (as shown in Section 6), standard global MMD achieves excellent transfer efficiency (+7.7% over baseline) without the complexity of regime-conditional methods. This trade-off between theoretical sophistication and practical simplicity is explored in detail in Section 6, where we adopt Standard MMD as the primary method for transferring US factor crowding insights to global markets.
